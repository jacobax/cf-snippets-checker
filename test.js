...
async function checkSnippets(zone, token) {
  ...
  try {
    ...
    if (status === 200) {
      ...
      result.msg = result.enabled ? "✅ 已开通 (200 OK)" : "❌ 状态异常 (200 Fail)";
    ...
    // 2. 失败，强制输出状态码
      ...
      try {
          ...
          // 如果响应体包含内容，则输出前50个字符
          if (rawText.length > 0) {
              debugMsg += ` | 响应文本: ${rawText.substring(0, 50)}...`;
          }
      } catch(e) {
          // 忽略 body 读取错误
      }
      ...
      // 针对关键状态码进行提示
      if (status === 403) {
          result.msg = `🔴 权限拒绝 (403). ${debugMsg}`;
      } else if (status === 404) {
          result.msg = `🟡 接口缺失 (404). ${debugMsg}`;
      } else {
          result.msg = `❌ ${debugMsg}`;
      }
    }
    ...
    <tr class="border-b border-gray-200/60 hover:bg-blue-50/60 transition-colors duration-150">
      ...
      <td class="py-4 px-6 text-sm font-medium">
        ${r.enabled 
          ...
          // *** 关键修改: 当未开通时，显示 r.msg 的内容 ***
          : `<span class="text-xs text-gray-500">${r.msg}</span>` 
        }
      </td>
    </tr>
  `).join("");
